...
var MAX_CODEX_PROMPT_CHARS = 2e4;
var MAX_CODEX_OUTPUT_CHARS = 2e5;
var MAX_CODEX_CONCURRENCY = 1;
var CODEX_EXEC_TIMEOUT_MS = 18e4;
var CODEX_PROCESS_KILL_GRACE_MS = 5e3;
...
var McpIntegrationServer = class {
  constructor(config2, credentials, codexAuth) {
    this.config = config2;
...
`);
  ...
  async handleCodexMcp(res, body, signal) {
  ...
  async handleCodexPrompt(res, request, signal) {
...
function managedMcpServerConfig(config2) {
  ...
  return {
    ...
    codex: {
      url: `http://127.0.0.1:${config2.mcpPort}/mcp/codex`,
      transport: "streamable-http",
      headers,
      timeout: Math.ceil(CODEX_EXEC_TIMEOUT_MS / 1e3) + 15,
      connectTimeout: 10,
      supportsParallelToolCalls: false
    }
  };
}
function codexToolsListResponse(id) {
  return {
    jsonrpc: "2.0",
    id,
    result: {
      tools: [
        {
          name: "codex_prompt",
          description: "Ask the deployment's connected OpenAI Codex account for text-only assistance. The managed wrapper disables local shell, browser, project rule, and user-config tools before invoking Codex.",
          inputSchema: {
            type: "object",
            properties: {
...
function codexPromptArgument(request) {
  const args = toolArguments(request);
  const prompt = args?.prompt;
  return typeof prompt === "string" && prompt.trim() && prompt.length <= MAX_CODEX_PROMPT_CHARS ? prompt : void 0;
}
function toolArguments(request) {
  if (!request.params || typeof request.params !== "object" || Array.isArray(request.params)) return void 0;
  const params = request.params;
  const args = params.arguments;
  return args && typeof args === "object" && !Array.isArray(args) ? args : void 0;
}
function mcpToolText(id, text, isError) {
  return {
    jsonrpc: "2.0",
    id,
    result: {
...
async function runTrustedCodexPrompt(params) {
  ...
  try {
    await fs.mkdir(workDir, { mode: 448 });
    const prompt = [
...
function truncateCodexOutput(value) {
...
// src/mlclaw-space-runtime/codex-auth.ts
...
// src/mlclaw-space-runtime/codex-auth.ts
var DEFAULT_SYNC_DEBOUNCE_MS = 1e3;
var CodexAuthManager = class {
  constructor(config2) {
    this.config = config2;
  }
  watchAbort;
  ...
  async startSync() {
    ...
    watchAuthFile(this.authJsonFile(), this.watchAbort.signal, () => this.schedulePersist()).catch((error) => {
      if (!(error instanceof Error && error.name === "AbortError")) {
        process.stderr.write(`[mlclaw] Codex auth watcher stopped: ${formatError2(error)}
`);
      }
    });
  }
  ...
  schedulePersist() {
    ...
    this.syncTimer = setTimeout(() => {
      this.syncTimer = void 0;
      this.syncTail = this.syncTail.then(() => this.persistCurrentAuth()).catch((error) => {
        process.stderr.write(`[mlclaw] failed to persist Codex auth refresh: ${formatError2(error)}
`);
      });
  ...
  async persistCurrentAuth() {
    ...
    const document = encodeCodexAuthDocument({ authJson, now: /* @__PURE__ */ new Date() });
  ...
  async saveEncryptedDocument(document) {
    const mountedFile = this.mountedStoreFile();
    ...
    if (!mountedFile) {
      await this.uploadEncryptedObject(await fs6.readFile(target, "utf8"));
    }
  }
  async deleteEncryptedDocument() {
    const mountedFile = this.mountedStoreFile();
  ...
  async uploadEncryptedObject(encrypted) {
  ...
  async ensureCodexHome() {
    await fs6.mkdir(this.config.codexHome, { recursive: true, mode: 448 });
    await fs6.chmod(this.config.codexHome, 448);
  }
  async writeCodexConfig() {
    const file = path6.join(this.config.codexHome, "config.toml");
    await writePrivateRawFile(file, 'cli_auth_credentials_store = "file"\nforced_login_method = "chatgpt"\n');
  }
  async readAuthJson() {
    try {
      const raw2 = await fs6.readFile(this.authJsonFile(), "utf8");
      const parsed = JSON.parse(raw2);
      return parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed : void 0;
    } catch (error) {
      if (isNotFound2(error)) return void 0;
      throw error;
    }
  }
  async writeAuthJson(authJson) {
    const file = this.authJsonFile();
    await writePrivateRawFile(file, `${stableJson(authJson)}
`);
  }
  authJsonFile() {
    return path6.join(this.config.codexHome, "auth.json");
  }
};
async function watchAuthFile(file, signal, onChange) {
  const directory = path6.dirname(file);
  await fs6.mkdir(directory, { recursive: true, mode: 448 });
  for await (const event of fs6.watch(directory, { signal })) {
    if (event.filename === path6.basename(file)) {
      onChange();
    }
  }
}
...
var SpaceRuntimeServer = class {
  constructor(config2, options = {}) {
    ...
    this.codexAuth = new CodexAuthManager(config2);
    this.mcpIntegrations = new McpIntegrationServer(config2, this.mcpCredentials, this.codexAuth);
    const credentialSlot = integrationCredentialSlot(config2);
  ...
  async start() {
    ...
    const server2 = http3.createServer((req, res) => {
      this.handle(req, res).catch((err) => {
        ...
        process.stderr.write(`[mlclaw] request failed: ${formatError3(err)}
`);
        if (!res.headersSent) {
    ...
    server2.on("upgrade", (req, socket, head) => {
      ...
      try {
        ...
        process.stderr.write(`[mlclaw] websocket upgrade failed: ${formatError3(err)}
`);
        rejectWebSocket(netSocket);
      }
    });
