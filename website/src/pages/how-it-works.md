...
title: How it works
description: The architecture of an ML Claw deployment, from browser authentication and durable state to unYOLO token isolation.
---
...
Those approvals happen inside the gateway. OpenClaw's unYOLO plugin adds an Approvals tab where administrators inspect, approve, deny, or revoke parked requests. The operator tokens behind those decisions live in backend-only files and are never sent to the browser or to OpenClaw.
