...
describe("runtime image Dockerfile", () => {
  it("healthchecks the ML Claw gateway port", async () => {
    ...
    expect(OPENCLAW_BASE_IMAGE).toBe("ghcr.io/osolmaz/openclaw:f8c76fd0595");
    expect(OPENCLAW_BUILD_ID).toBe("f8c76fd0595");
    expect(dockerfile).toContain(`ARG OPENCLAW_BASE_IMAGE=${OPENCLAW_BASE_IMAGE}`);
    ...
    expect(DEFAULT_RUNTIME_IMAGE).toBe(
      `ghcr.io/osolmaz/mlclaw:${PACKAGE_VERSION}-openclaw-${OPENCLAW_VERSION}-${OPENCLAW_BUILD_ID}`,
    );
