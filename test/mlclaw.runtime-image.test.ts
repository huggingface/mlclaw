...
describe("runtime image Dockerfile", () => {
  it("healthchecks the ML Claw gateway port", async () => {
    ...
    expect(dockerfile).toContain("FROM --platform=$BUILDPLATFORM node:24-bookworm-slim AS sync-build");
  ...
  it("leaves workspace tooling seeding to the bootstrap-aware runtime", async () => {
    ...
    expect(entrypoint).toContain('rm -rf -- \\\n    "$HF_BROKER_STATE_DIR/grants"');
    expect(entrypoint).toContain('"$HF_BROKER_STATE_DIR/operations"');
